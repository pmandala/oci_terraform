...
#
# ex:
# oci-curl iaas get "/20160918/instances?compartmentId=some-compartment-ocid"
# oci-curl iaas post ./request.json "/20160918/vcns"
# oci-curl iaas get "/20160918/instances?compartmentId=ocid1.compartment.oc1..aaaaaaaallacqbo7kb6qavrnyj4y6x4mjwsl3pzfdrxxkt3kw5s2e73m2s4a"

function oci-curl {
	local tenancyId="${TF_VAR_tenancy_ocid}";
	local authUserId="${TF_VAR_user_ocid}";
	local keyFingerprint="${TF_VAR_fingerprint}";
	local privateKeyPath="${TF_VAR_private_key_path}";
	local domain_name="${TF_VAR_domain_name_override:-oraclecloud.com}";

	local alg=rsa-sha256
	local sigVersion="1"
	local now="$(LC_ALL=C \date -u "+%a, %d %h %Y %H:%M:%S GMT")"
	local host="$1.$TF_VAR_region.$domain_name"
	local method=$2
	local extra_args
